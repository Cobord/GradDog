...
def plot_derivative(
    ...
    xlabel="x",
    ylabel="y",
    plot_title="Derivative",
    verbose=False,
):
    """
    Plot the derivative of a function between xmin and xmax,
    using n_pts linearly spaced points to evaluate it.

    Inputs:
    function[function]: the function of which you'd like to plot the derivative
        (must only take single input)
    xmin: lower bound on which to calculate derivative
    xmax: upper bound on which to calculate derivative
    n_pts [int](Default: 100): how many points to use when evaluating derivative
        (more = better resolution but slower)
...
def find_extrema_firstorder(
    ...
    n_pts [int](Default: 100): how many points to use when evaluating derivative
        (more = better resolution but slower)
    tolerance [float](Default: 1e-10): how close to zero should a value be
        before it's considered an extrema?
    ...
    if len(zeroidx) != 0:
        return xs[zeroidx]
    decreasing_idx = np.where(y_ders < tolerance)[0].astype(int)
...
def find_increasing(function, xmin, xmax, n_pts=100, verbose=False):
    ...
    Inputs:
    function[function]: the function that you want to locate increasing regions on
        (one scalar input)
    xmin: lower bound of interval
    xmax: upper bound of interval
    n_pts (default = 100): how many points to use when evaluating derivative
        (more = better resolution but slower)
...
def find_decreasing(function, xmin, xmax, n_pts=100, verbose=False):
    ...
    Inputs:
    function[function]: the function that you want to locate decreasing regions on
        (one scalar input)
    xmin: lower bound of interval
    xmax: upper bound of interval
    n_pts (default = 100): how many points to use when evaluating derivative
...
def plot_with_tangent_line(
    ...
    xlabel="x",
    ylabel="y",
    plot_title="Function with tangent line",
    verbose=False,
):
    """
    Plot the a function between xmin and xmax, with a tangent line at xtangent,
    using n_pts linearly spaced points to evaluate it.
    ...
    xmin: lower bound on which to calculate derivative
    xmax: upper bound on which to calculate derivative
    n_pts [int](Default: 100): how many points to use when plotting function
        (more = better resolution but slower)
    figsize [tuple](Default: (6,6)): figsize in inches (see matplotlib documentation)
    xlabel [string](Default: 'x'): Label for x axis of plot
    ylabel [string](Default: 'y'): Label for y axis of plot
