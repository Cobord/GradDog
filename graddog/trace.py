...
class Trace:
    ...
    def __init__(
        ...
        # name by default is the trace name,
        # but can be changed to something like 'Cost' or 'Objective' or 'Loss'
    ...
    def __rtruediv__(self, other):
        ...
        """
        return one_parent(
            self, math.Ops.div_R, other, formula=f"{other}/{self._trace_name}"
        )
    ...
    def __rpow__(self, other):
        ...
        """
        return one_parent(
            self, math.Ops.exp, other, formula=f"{other}^{self._trace_name}"
        )
...
def two_parents(t1: Trace, op, t2, formula=None):
    ...
    try:
        ...
        if isinstance(t2, numbers.Number):
            # when t2 is actually a constant, not a trace,
            # and this should really be a one parent trace
