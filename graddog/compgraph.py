...
class CompGraph:
    ...
    class __CompGraph:
        ...
        def reset(self):
            ...
            # store the graph connections in two separate dictionaries,
            # ins (AKA parents) and outs (AKA children)
            self.outs = {}
            self.ins = {}
        ...
        def get_variable_row(self, var_name):
            return (
                int(
                    self.table.loc[self.table["formula"] == var_name][
                        "trace_name"
                    ].iloc[0][1:]
                )
                - 1
        ...
        def get_trace_name(self, var_name):
            ...
            return self.table.loc[self.table["formula"] == var_name]["trace_name"].iloc[
                0
            ]
        ...
        def forward_mode_der(self, output, verbose=False):
            """
            step FORWARDS through the trace table, calculate derivatives
            along the way in trace_derivs

            The gradient for each trace is computed as the vector `d_v_d_variables`

            Returns the Jacobian matrix of derivatives df_i/dx_j for each output function
            f_i w.r.t. each input variable x_j
            ...
            self.trace_derivs = {
                self.get_trace_name(x): np.eye(self.num_vars)[i, :]
            ...
            for row in range(self.num_vars, self.size):
                ...
                d_v_d_parents = np.array(
                    [[self.partials[v][in_] for in_ in self.ins[v]]]
                )

                d_parents_d_variables = np.vstack(
                    [self.trace_derivs[in_] for in_ in self.ins[v]]
                )
        ...
        def reverse_mode_der(self, output, verbose=False):
            """
            step BACKWARDS through the trace table, calculate derivatives
            along the way in trace_derivs
            ...
            self.trace_derivs = {
                x: np.eye(self.num_outputs())[:, i].reshape(-1, 1)
            ...
            for row in reversed(range(self.size)):
                ...
                if not is_output:
                    ...
                    if self.outs[v] == []:
                        if verbose:
                            print(
                                f"{v} was a variable with no effect on any of the outputs"
                            )
                        ...
                        d_outputs_d_children = np.hstack(
                            [self.trace_derivs[out_] for out_ in self.outs[v]]
                        )

                        d_children_d_v = np.array(
                            [[self.partials[out_][v] for out_ in self.outs[v]]]
                        )
            ...
            return np.hstack(
                [
                    self.trace_derivs[x]
                    for x in list(map(lambda x: self.get_trace_name(x), self.variables))
                ]
            )
        ...
        def hessian(self, output, verbose=False):
            ...
            # h[k][v_i][v_j] gives the 2nd derivative of the output
            # w.r.t. v_i and v_j during iteration k of the algorithm
            h = {
                k: {f"v{j+1}": {f"v{i+1}": 0.0 for i in range(l)} for j in range(l)}
            ...
            for k in reversed(range(m + 1, l + 1)):
                ...
                for v_i, v_j in combinations_with_replacement(S[k], 2):
                    ...
                    if v_i in self.ins[v_k] and v_j in self.ins[v_k]:
                        double_deriv = (
                            math.double_deriv(
                                self.traces[v_k], self.traces[v_i], self.traces[v_j]
                            )
                            * v_bar[v_k]
                        )
                    else:
                    ...
                    new_value = (
                        h[k + 1][v_i][v_j]
                        + self.partials[v_k][v_i] * h[k + 1][v_j][v_k]
                        + self.partials[v_k][v_j] * h[k + 1][v_i][v_k]
                        + self.partials[v_k][v_i]
                        * self.partials[v_k][v_j]
                        * h[k + 1][v_k][v_k]
                        + double_deriv
                    )
            ...
            h = np.array(
                [[h[m + 1][f"v{i+1}"][f"v{j+1}"] for j in range(m)] for i in range(m)]
            )
            return j, h
